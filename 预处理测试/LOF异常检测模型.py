...
class 完整拟合模型:
    ...
    def 数据预处理(self):
        ...
        lof = LocalOutlierFactor(
            n_neighbors=self.LOF邻居数量,
            contamination=self.LOF异常值比例
        )
        异常值索引 = lof.fit_predict(df.select_dtypes(include=['number'])) == -1
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(photos_dir, f'LOF_前处理对比_{current_time}.png'))
        plt.close()
        ...
        for i, col in enumerate(y.columns):
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(photos_dir, f'LOF_{col}_三维散点图_{current_time}.png'))
            plt.close()
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(photos_dir, f'LOF_异常值可视化_{current_time}.png'))
        plt.close()
    ...
    def 保存结果(self):
        ...
        result_path = os.path.join(self.结果目录, f'LOF_{current_time}.csv')

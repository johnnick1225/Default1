...
models = [GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10) 
          for _ in range(Y_train_scaled.shape[1])]
...
# === 预测 ===
Y_train_pred = np.array([model.predict(X_train_scaled) for model in models]).T
...
result_path = os.path.join(result_dir, f'高斯过程回归_{current_time}.csv')
metrics = []
